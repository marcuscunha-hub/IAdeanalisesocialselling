...
export default function SellerProfilePage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="px-8 py-10">
      {/* Breadcrumb */}
      <div className="flex items-center gap-2 text-xs text-beige-100 mb-6">
        <Link href="/dashboard/vendedores" className="hover:text-forest transition">Vendedores</Link>
        <span>/</span>
        <span className="text-forest">{seller.name}</span>
      </div>
      ...
      {demo && (
        <div className="bg-beige-25 border border-beige-50 rounded-xl px-5 py-3 mb-6 text-sm text-beige-100">
      ...
      <div className="mb-6 flex items-center justify-between">
        <h2 className="font-heading text-2xl text-forest">Último relatório</h2>
        <div className="flex items-center gap-2 text-beige-100 text-sm">
          Score geral:
          <span className="font-heading text-2xl text-forest ml-1">{report.score}</span>
          <span className="text-beige-100">/100</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
        {Object.entries(report.stages).map(([key, stage]) => {
          ...
          return (
            <div key={key} className="bg-white border border-beige-50 rounded-2xl p-5">
              <p className="text-xs font-medium tracking-widest uppercase text-beige-100 mb-2">{STAGE_LABELS[key]}</p>
              <p className={`font-heading text-3xl ${textColor}`}>{stage.score}<span className="text-beige-100 text-base">/100</span></p>
              ...
              <p className="mt-3 text-xs text-forest/60 leading-relaxed line-clamp-2">{stage.feedback}</p>
            </div>
          );
        })}
      </div>
      ...
      <div className="mb-5 flex items-center justify-between">
        <h2 className="font-heading text-2xl text-forest">Histórico</h2>
        <span className="text-xs bg-beige-25 border border-beige-50 rounded-full px-3 py-1 text-beige-100">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-beige-50">
              <th className="text-left px-6 py-4 text-xs font-medium tracking-widest uppercase text-beige-100">Data</th>
              <th className="text-left px-6 py-4 text-xs font-medium tracking-widest uppercase text-beige-100">Período</th>
              <th className="text-left px-6 py-4 text-xs font-medium tracking-widest uppercase text-beige-100">Conversas</th>
              <th className="text-left px-6 py-4 text-xs font-medium tracking-widest uppercase text-beige-100">Score</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-beige-50">
            {MOCK_HISTORY.map((h, i) => (
              <tr key={i} className="hover:bg-beige/50 transition-colors">
                <td className="px-6 py-4 text-forest/70">{h.date}</td>
                <td className="px-6 py-4 text-forest/70">{h.period}</td>
                <td className="px-6 py-4 text-forest/70">{h.conversas}</td>
                <td className="px-6 py-4">
                  <span className={`font-heading text-lg ${h.score >= 70 ? "text-olive" : h.score >= 50 ? "text-olive/70" : "text-coral"}`}>
                    {h.score}
                  </span>
                  <span className="text-beige-100 text-xs">/100</span>
                </td>
              </tr>
            ))}
          </tbody>
