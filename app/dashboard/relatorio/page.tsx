...
function RelatorioContent() {
  ...
  return (
    <div className="px-8 py-10">
      <div className="mb-8">
        <p className="text-xs font-medium tracking-widest uppercase text-beige-100 mb-1">Análise com IA</p>
        <h1 className="font-heading text-4xl text-forest">Gerar relatório</h1>
        <p className="text-beige-100 text-sm mt-1">Selecione o vendedor e o período para analisar as conversas.</p>
      </div>

      {/* Filter card */}
      <div className="bg-white border border-beige-50 rounded-2xl p-6 flex flex-col sm:flex-row gap-4 items-stretch sm:items-end mb-8">
        <div className="flex-1">
          <p className="text-xs font-medium tracking-widest uppercase text-beige-100 mb-2">Vendedor</p>
        ...
        <div className="sm:w-44">
          <p className="text-xs font-medium tracking-widest uppercase text-beige-100 mb-2">Período</p>
        ...
        <button onClick={generateReport} disabled={!igAccountId || loading}
          className="bg-forest text-lime rounded-lg px-6 py-2.5 text-sm font-medium hover:bg-moss transition disabled:opacity-40 whitespace-nowrap flex items-center gap-2">
      ...
      {!report && !loading && !error && (
        <div className="text-center py-24 border border-dashed border-beige-50 rounded-2xl">
          <p className="font-heading text-3xl text-forest/20 mb-2">Nenhum relatório gerado</p>
          <p className="text-sm text-beige-100">Clique em "Gerar relatório" para começar a análise</p>
        </div>
      )}
        ...
        <div className="space-y-5">
            ...
            {Object.entries(report.stages).map(([key, stage]) => (
              <div key={key} className="bg-white border border-beige-50 rounded-2xl p-6">
                <div className="flex items-start justify-between mb-1">
                  <p className="text-xs font-medium tracking-widest uppercase text-beige-100">{STAGE_LABELS[key] ?? key}</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-white border border-beige-50 rounded-2xl p-6">
              <p className="text-xs font-medium tracking-widest uppercase text-beige-100 mb-4">Pontos positivos</p>
            ...
            <div className="bg-white border border-beige-50 rounded-2xl p-6">
              <p className="text-xs font-medium tracking-widest uppercase text-beige-100 mb-4">Pontos de melhoria</p>
              <ul className="space-y-3">
...
export default function RelatorioPage() {
  return (
    <Suspense fallback={<div className="px-8 py-10 text-beige-100 text-sm">Carregando...</div>}>
      <RelatorioContent />
    </Suspense>
  );
}
